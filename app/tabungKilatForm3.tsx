...
export default function FundraisingVerificationScreen() {
  ...
  const handleComplete = async () => {
    ...
    try{
    setLoading(true);
    await new Promise(resolve => setTimeout(resolve, 1500)); // 👈 add before DB code (for testing)
...
const docSnap = await getDoc(userDocRef); //retrieve doc to check if already existed
      ...
      if (docSnap.exists()) {
        await updateDoc(userDocRef, {
          requestTabungKilat: arrayUnion({ //to save data in array form or add new conversation without removing previous one
    ...
    router.replace('/completeKilatForm');
        ...
        setLoading(false); // make sure it stops
      }
  };
